...
requestInstance.interceptors.response.use(
  (response) => {
    ...
    if (requestResult.code === 200) {
      ...
      if (response.headers.options?.isCatchAllErrorCode === true) {
        ...
        response.headers.options?.catchErrorCodeSet?.has(requestResult.code)
      ) {
        // 拦截处理
      } else {
            ...
            refreshToken()
              ...
              .catch((error) => {
                ...
                requestResult.message =
                  CommonRequestErrorEnum.REFRRSHTOKEN_INVALID;
                return Promise.reject(requestResult);
              });
          } else {
  ...
  (error) => {
    ...
    if (error.response) {
      ...
      if (
        error.config.headers.retryRequestCount == undefined ||
        error.config.headers.retryRequestCount < retryRequestMaxCount
      ) {
